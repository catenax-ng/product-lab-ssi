...
public class RemoteDocumentLoader implements DocumentLoader {
  ...
  private static final Map<URI, Document> staticCache = new HashMap<>();
  ...
  @Getter @Setter private boolean enableHttps = true;
  @Getter @Setter private boolean enableFile = false;
  @Getter @Setter private Map<URI, Document> localCache = staticCache;
  ...
  public RemoteDocumentLoader(Map<URI, Document> localCache) {
  ...
  public Document loadDocument(URI url, DocumentLoaderOptions options) throws JsonLdError {
    ...
    if (this.isEnableHttp() && "http".equalsIgnoreCase(url.getScheme())) {
      ...
      document = this.getRemoteCache() == null ? null : this.getRemoteCache().getIfPresent(url);
      if (document == null) {
        document = httpLoader.loadDocument(url, options);
        if (this.getRemoteCache() != null) this.getRemoteCache().put(url, document);
      }
    } else if (this.isEnableHttps() && "https".equalsIgnoreCase(url.getScheme())) {
    ...
    if (this.isEnableLocalCache() && document != null) {
      this.getLocalCache().put(url, document);
    }
    return document;
  }
}
